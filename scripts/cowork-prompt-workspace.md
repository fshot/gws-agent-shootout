# GWS Agent Shootout — Config A Test Run (Workspace)
...
- **Other account** (for sharing/emailing): fshotwell@gmail.com
- **Today's date**: 2026-04-16
- **Artifact prefix**: `[GWS-A]`

## Important: Account verification

Before starting tests, verify which Google account the MCP server is authenticated as. Run a Drive search or Gmail search and check the results match the workspace account (fshotwell@cruxcapacity.com), NOT the gmail account. If the MCP server is authenticated as the wrong account, report this immediately and stop — do not proceed with tests against the wrong account.
...
Share the file from #2 with `fshotwell@gmail.com` as Viewer.
...
### #12 — Gmail: send
Send an email to `fshotwell@gmail.com` with:
